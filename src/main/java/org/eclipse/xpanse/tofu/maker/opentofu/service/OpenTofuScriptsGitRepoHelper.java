...
import static org.eclipse.xpanse.tofu.maker.opentofu.service.OpenTofuScriptsDirectoryHelper.TF_SCRIPT_FILE_EXTENSION;
...
@Slf4j
@Component
public class OpenTofuScriptsGitRepoHelper {
    ...
    public List<File> checkoutScripts(String workspace, OpenTofuScriptsGitRepoDetails scriptsRepo) {
        log.info(
    ...
    private List<File> getSourceFiles(String workspace, OpenTofuScriptsGitRepoDetails scriptsRepo) {
    ...
    private void validateIfFolderContainsOpenTofuScripts(
            List<File> files, OpenTofuScriptsGitRepoDetails scriptsRepo) {
        boolean isScriptsExisted =
                files.stream().anyMatch(file -> file.getName().endsWith(TF_SCRIPT_FILE_EXTENSION));
        if (!isScriptsExisted) {
            throw new InvalidOpenTofuRequestException(
