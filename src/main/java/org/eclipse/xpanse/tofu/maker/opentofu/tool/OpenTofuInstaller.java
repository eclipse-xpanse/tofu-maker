...
public class OpenTofuInstaller {
    ...
    /**
     * Find the executable binary path of the OpenTofu tool that matches the required version.
     * If no matching executable binary is found, install the OpenTofu tool with the required
     ...
     */
    @Retryable(retryFor = InvalidOpenTofuToolException.class,
    ...
    private String getBestAvailableVersionMatchingRequiredVersion(String requiredOperator,
        ...
        if (StringUtils.isNotBlank(bestAvailableVersion)) {
            log.info("Found the best available version {} for OpenTofu by the required version "
                    + "{}.", bestAvailableVersion, requiredOperator + requiredNumber);
            return bestAvailableVersion;
        }
        String errorMsg = String.format("Failed to find available versions for OpenTofu by the "
